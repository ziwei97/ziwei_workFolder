...
class AutoSlideGen:
    ...
    def generate_slide(self):
        ...
        for dirs, subdir, files in os.walk(self.imgLoc):
            if not subdir:
                mask_imgs = [ele for ele in files if
                             'Algorithm_Mask' in ele]  # 'Mask_Truth' in ele and 'HoleDeleted' in ele]

                if len(mask_imgs) > 0:

                    mask_img = mask_imgs[0]
                    truth_img = \
                    [ele for ele in files if ('FinalTruth_' in ele and '.xcf' not in ele) and ele not in [mask_img]][0]
...
src_path = '/Users/ziweishi/Documents/burn_mask19'
maskKeyWord = 'Algorithm_Mask_'
centers = os.listdir(src_path)
if '.DS_Store' in centers:
    centers.remove('.DS_Store')
# subjects = os.listdir(src_path)

dst_path = '/Users/ziweishi/Documents/Review_ppts19'
